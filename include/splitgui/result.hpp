...
namespace SplitGui {
    ...
    #define TRYC(type, name, resultValue) SplitGui::ResultValue<type> name = resultValue; if (name.result != SplitGui::Result::eSuccess) { return (int)name.result; }
    #define TRYR(name, result) SplitGui::Result name = result; if (name != SplitGui::Result::eSuccess) { return name; }
    #define TRYRC(name, result) SplitGui::Result name = result; if (name != SplitGui::Result::eSuccess) { return (int)name; }
